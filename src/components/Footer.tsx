...
export function Footer() {
  ...
  const contactInfo = [
    {
      icon: <Mail size={18} className="text-mint" />,
      text: "oxerfy@gmail.com",
      href: "mailto:oxerfy@gmail.com",
    },
    {
      icon: <MapPin size={18} className="text-mint" />,
      text: "Dhaka, Bangladesh (Remote)",
    },
  ];
  ...
  return (
    <footer className="bg-black/40 backdrop-blur-lg relative h-fit rounded-t-3xl overflow-hidden mt-8 z-10 border-t border-white/10">
      {/* Intensified Aurora for Contact */}
      <div className="absolute inset-0 opacity-60 mix-blend-screen pointer-events-none">
        <motion.div
          ...
          className="absolute top-0 left-1/2 -translate-x-1/2 w-[80vw] h-[80vw] rounded-full bg-teal/40 blur-[150px]"
        />
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 pt-32 pb-14 z-[60] relative pointer-events-none">
        ...
        <div id="contact" className="mb-32 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-start pointer-events-auto">
          <motion.div
            ...
            className="lg:col-span-5 text-left lg:sticky lg:top-32 space-y-10"
          >
            <div>
            ...
            <div className="grid grid-cols-1 gap-4 max-w-sm">
              <a 
                href="mailto:contact@oxerfy.com" 
          ...
          <div className="lg:col-span-7 flex flex-col gap-8 w-full">
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
              className="glass-card p-10 xl:p-12 flex-1 w-full"
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                  <div className="flex flex-col gap-2">
                    ...
                    <select required defaultValue="" className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-cream focus:outline-none focus:border-mint w-full">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <textarea id="message" rows={4} placeholder="Tell us about your goals, timeline, and budget..." required className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-mint transition-colors resize-none w-full"></textarea>
                </div>
              ...
              <ul className="space-y-3">
                {section.links.map((link) => (
                  <li key={link.label} className="relative w-fit">
                    <a
                      href={link.href}
                      className="inline-block text-cream/70 hover:text-mint hover:translate-x-1.5 transition-all duration-300 ease-out cursor-none"
                    >
                      {link.label}
                    </a>
                    {link.pulse && (
                      <span className="absolute top-1.5 -right-3.5 w-1.5 h-1.5 rounded-full bg-mint animate-[pulse_2s_cubic-bezier(0.4,0,0.6,1)_infinite] shadow-[0_0_8px_rgba(32,189,90,0.8)]"></span>
                    )}
                  </li>
                ))}
              </ul>
              ...
              {contactInfo.map((item, i) => (
                <li key={i} className="flex items-center space-x-3 text-cream/70 group">
                  ...
                  {item.href ? (
                    <a
                      href={item.href}
                      className="inline-block hover:text-mint hover:translate-x-1.5 transition-all duration-300 ease-out"
                    >
                      {item.text}
                    </a>
                  ) : (
                    <span className="inline-block hover:text-mint hover:translate-x-1.5 transition-all duration-300 ease-out">
                      {item.text}
                    </span>
                  )}
                </li>
          ...
          <div className="flex space-x-4">
            {socialLinks.map(({ icon, label, href }) => (
              <a
                ...
                href={href}
                aria-label={label}
                className="flex items-center justify-center w-10 h-10 rounded-full bg-white/5 border border-white/5 text-cream/70 hover:text-mint hover:bg-white/10 hover:border-mint/30 hover:-translate-y-1 hover:scale-105 transition-all duration-300 ease-out cursor-none shadow-lg hover:shadow-mint/20"
              >
                {icon}
              </a>
            ))}
